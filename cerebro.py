...
import pickle
import config
from utils import tf_execute_helper, pytorch_execute_helper, uuid, mst_identifier, preload_data_helper
...
def update_mst_evaluation_state(epoch_mst_evaluation_state, mst_evaluation_state):
    ...
    for mode in ["train", "valid"]:
        for mst_id in epoch_mst_evaluation_state:
            mst_evaluation_state[mst_id][mode + "_loss"].append(
                sum(epoch_mst_evaluation_state[mst_id][mode + "_loss"]) / len(
                    epoch_mst_evaluation_state[mst_id][mode + "_loss"]))
            mst_evaluation_state[mst_id][mode + "_error"].append(
                sum(epoch_mst_evaluation_state[mst_id][mode + "_error"]) / len(
                    epoch_mst_evaluation_state[mst_id][mode + "_error"]))
            if mode == "train":
...
def evaluate_msts(mst_eval_fn, mst_evaluation_state, current_msts, ckpt_root):
    ...
    for mst_id, new_mst in zip(range(id_max + 1, id_max + 1 + len(new_msts)), new_msts):
        ...
        if 'init_mst' in new_mst:
            ...
            mst_evaluation_state[mst_id]['valid_error'] = [x for x in
                                                           mst_evaluation_state[new_mst['init_mst_id']]['valid_error']]
            mst_evaluation_state[mst_id]['train_error'] = [x for x in
                                                           mst_evaluation_state[new_mst['init_mst_id']]['train_error']]
            mst_evaluation_state[mst_id]['valid_loss'] = [x for x in
                                                          mst_evaluation_state[new_mst['init_mst_id']]['valid_loss']]
            mst_evaluation_state[mst_id]['train_loss'] = [x for x in
                                                          mst_evaluation_state[new_mst['init_mst_id']]['train_loss']]
...
def schedule(worker_ips, train_partitions, valid_partitions, train_availability, valid_availability,
    ...
    def _get_runnable_unit(epoch_units, w, availability, epoch_mst_execution_state):
        random.shuffle(epoch_units)
        for idx, (mst_id, mst, partition) in enumerate(epoch_units):
            if availability[w][partition] == 1 and (epoch_mst_execution_state[mst_id] == False):
                del epoch_units[idx]
                ...
                for w in [w for w in range(W) if w in workers]:
                    ...
                    try:
                        if epoch_machine_state[w] is None:
                            mst_id, mst, p = _get_runnable_unit(epoch_units, w, availability, epoch_mst_execution_state)
                            if mst_id != -1:
